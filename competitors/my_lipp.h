#ifndef __MY_LIPP_H__
#define __MY_LIPP_H__
...
public:
    ...
    MyLIPP(double BUILD_LR_REMAIN = 0, bool QUIET = true)
...
public:
    bool find(const T& key, P& value) const {
        ...
        while (true) {
            int pos = PREDICT_POS(node, key);
            // Child traversal is the common case (inner nodes); bias the branch predictor
            if (__builtin_expect(BITMAP_GET(node->child_bitmap, pos) == 1, 1)) {
                node = node->items[pos].comp.child;
            } else {
    ...
    class LIPPIterator {
        using lipp_type = MyLIPP<T, P, USE_FMCD>;
